    ...
    public void migrateSchema() {
        ...
        try (Connection conn = dataSource.getConnection(); Statement stmt = conn.createStatement()) {
            ...
            String[] checkpointColumns = {
                    ...
                    "ALTER TABLE repo_mappings ADD COLUMN IF NOT EXISTS lfs_scanned_tip_oids CLOB",
                    "ALTER TABLE repo_mappings ADD COLUMN IF NOT EXISTS last_source_tip_fingerprint VARCHAR(64)",
                    "ALTER TABLE repo_mappings ADD COLUMN IF NOT EXISTS last_dest_tip_fingerprint VARCHAR(64)",
                    "ALTER TABLE repo_mappings ADD COLUMN IF NOT EXISTS last_pr_list_completed_at TIMESTAMP",
                    "ALTER TABLE repo_mappings ADD COLUMN IF NOT EXISTS last_release_sync_at TIMESTAMP",
                    "ALTER TABLE repo_mappings ADD COLUMN IF NOT EXISTS fork_pr_miss_json CLOB",
            ...
            String[] syncJobColumns = {
                    ...
                    "ALTER TABLE sync_jobs ADD COLUMN IF NOT EXISTS graphql_call_count INTEGER",
                    "ALTER TABLE sync_jobs ADD COLUMN IF NOT EXISTS graphql_points_used INTEGER",
                    "ALTER TABLE sync_jobs ADD COLUMN IF NOT EXISTS graphql_429_count INTEGER",
                    "ALTER TABLE sync_jobs ADD COLUMN IF NOT EXISTS git_read_bytes BIGINT",
                    "ALTER TABLE sync_jobs ADD COLUMN IF NOT EXISTS git_write_bytes BIGINT",
                    "ALTER TABLE sync_jobs ADD COLUMN IF NOT EXISTS lfs_synced_count INTEGER",
                    "ALTER TABLE sync_jobs ADD COLUMN IF NOT EXISTS provider_traffic_series_json CLOB",
                    "ALTER TABLE sync_jobs ADD COLUMN IF NOT EXISTS lfs_api_call_count INTEGER",
                    "ALTER TABLE sync_jobs ADD COLUMN IF NOT EXISTS lfs_transfer_http_count INTEGER",
            ...
            try {
                stmt.execute("""
                        UPDATE scm_credentials
                        SET installation_ids_json = CONCAT('[\"', installation_id, '\"]')
                        WHERE installation_id IS NOT NULL
                          AND TRIM(installation_id) <> ''
                          AND (installation_ids_json IS NULL OR TRIM(CAST(installation_ids_json AS VARCHAR)) = ''
                               OR TRIM(CAST(installation_ids_json AS VARCHAR)) = '[]')
                        """);
                log.info("Database migration: backfilled scm_credentials.installation_ids_json where needed.");
            } catch (Exception e) {
                log.debug("Schema migration notice (installation_ids_json backfill): {}", e.getMessage());
            }
            try {
                ...
                stmt.execute("ALTER TABLE repo_mappings ADD COLUMN IF NOT EXISTS source_installation_id VARCHAR(64)");
                stmt.execute("ALTER TABLE repo_mappings ADD COLUMN IF NOT EXISTS target_installation_id VARCHAR(64)");
            ...
            try {
                stmt.execute("""
                        CREATE TABLE IF NOT EXISTS feature_flags (
                            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                            public_repos_enabled BOOLEAN DEFAULT TRUE NOT NULL,
                            provider_gitlab_enabled BOOLEAN DEFAULT FALSE NOT NULL,
                            provider_bitbucket_enabled BOOLEAN DEFAULT FALSE NOT NULL,
                            provider_origin_enabled BOOLEAN DEFAULT FALSE NOT NULL,
                            provider_generic_enabled BOOLEAN DEFAULT FALSE NOT NULL,
                            updated_at TIMESTAMP
                        )
                        """);
                log.info("Database migration: verified feature_flags table.");
            } catch (Exception e) {
                log.debug("Schema migration notice (feature_flags): {}", e.getMessage());
            }
