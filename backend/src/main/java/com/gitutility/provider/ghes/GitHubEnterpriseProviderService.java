...
public class GitHubEnterpriseProviderService implements ScmProviderAdapter {
    ...
    public ReleaseLookup findReleaseByTag(String repoFullName, String tagName) {
        ...
        try {
            ...
            return findListedReleaseByTag(repoFullName, tagName, token, host);
        } catch (Exception e) {
    ...
    private ReleaseLookup findListedReleaseByTag(String repoFullName, String tagName, String token, String host) {
        try {
            HttpHeaders headers = createHeaders(token);
            String url = host + "/api/v3/repos/" + repoFullName + "/releases?per_page=100";
            ResponseEntity<String> resp = restTemplate.exchange(URI.create(url), HttpMethod.GET, new HttpEntity<>(headers), String.class);
            JsonNode releases = objectMapper.readTree(resp.getBody());
            if (!releases.isArray()) {
                return ReleaseLookup.missing();
    ...
    public boolean deleteRelease(String repoFullName, String externalId) {
        ...
        try {
            ...
            log.info("Deleted GHES release {} on {}", externalId, repoFullName);
            return true;
        } catch (HttpClientErrorException.NotFound ignored) {
            return true;
        } catch (HttpClientErrorException e) {
            throw new IllegalStateException("GHES release delete rejected (" + e.getStatusCode() + "): "
                    + e.getResponseBodyAsString(), e);
        } catch (Exception e) {
            throw new IllegalStateException("GHES release delete failed: " + e.getMessage(), e);
        }
    }
