...
import { submitBulkMigration, probeRepoHasCommits, listScmInstallations, BulkMirrorRequest, BulkMirrorResponse, BulkMirrorRow } from '../services/api';
import { RepoPickerModal } from './RepoPickerModal';
import { InfoTooltip } from './InfoTooltip';
import { normalizeRepoKey, findRepoCollision } from '../utils/repoUrl';
...
type DestMode = 'CREATE_DEST' | 'USE_EXISTING';

interface RowState {
  repo: GitHubRepoOption;
  destName: string;
  /** Option 2: chosen existing destination repo. */
...
export const BulkMigrationTab: React.FC<BulkMigrationTabProps> = ({
  ...
  const [mode, setMode] = useState<DestMode>('CREATE_DEST');
  const [rows, setRows] = useState<RowState[]>([]);
  const [pickerOpen, setPickerOpen] = useState<null | 'SOURCE' | 'DEST'>(null);
  const [destCredentialId, setDestCredentialId] = useState<number | ''>('');
  const [destOwner, setDestOwner] = useState('');
  const [destPrivate, setDestPrivate] = useState(true);
  ...
  React.useEffect(() => {
    ...
    const cred = githubCredentials.find((c) => c.id === Number(destCredentialId));
    let cancelled = false;
    ...
    void listScmInstallations(Number(destCredentialId))
      .then((installed) => {
        if (cancelled) return;
  ...
  const addDestRepos = (repos: GitHubRepoOption[]) => {
    const taken = new Set(rows.map((r) => normalizeRepoKey(r.destRepo?.cloneUrl)).filter(Boolean) as string[]);
    const pool = repos.filter((r) => !taken.has(normalizeRepoKey(r.cloneUrl)));
    const next = [...rows];
    for (let i = 0; i < next.length; i++) {
      if (next[i].destRepo) continue;
      const name = deriveRepoName(next[i].repo.cloneUrl);
      const match = pool.find((r) => deriveRepoName(r.cloneUrl) === name && !taken.has(normalizeRepoKey(r.cloneUrl)));
      const pick = match || pool.shift();
      if (pick) {
        next[i] = { ...next[i], destRepo: pick };
        taken.add(normalizeRepoKey(pick.cloneUrl));
      }
    }
    setRows(next);
    void probeDestinations(next);
  };
  ...
  const readyRows = rows.filter((r) => !r.excluded
    && (mode === 'CREATE_DEST' ? Boolean(r.destName.trim()) : Boolean(r.destRepo))
    && !(mode === 'USE_EXISTING' && r.destHasCommits && !r.includeNonEmptyDest));
    ...
    try {
      const payload: BulkMirrorRequest = {
        ...
        items: readyRows.map((r) => ({
          ...
          sourceVisibility: r.repo.isPrivate ? 'PRIVATE' : 'UNKNOWN',
          destName: mode === 'CREATE_DEST' ? r.destName.trim() : undefined,
          destUrl: mode === 'USE_EXISTING' ? r.destRepo!.cloneUrl : undefined,
          destCredentialId: mode === 'USE_EXISTING' ? r.destRepo!.credentialId : undefined,
          destInstallationId: mode === 'USE_EXISTING' ? r.destRepo!.installationId : undefined,
          includeNonEmptyDest: mode === 'USE_EXISTING' ? r.includeNonEmptyDest : undefined,
        })),
        branchPattern,
        syncDirection,
        trunkConflictPolicy,
        storageTier,
        active,
        destOwner: mode === 'CREATE_DEST' ? effectiveOwner : undefined,
        destCredentialId: mode === 'CREATE_DEST' ? (destCredentialId === '' ? undefined : Number(destCredentialId)) : undefined,
        destInstallationId: mode === 'CREATE_DEST'
          ? (selectedDestInstall?.installationId ?? destInstallationId)
          : undefined,
        destPrivate: mode === 'CREATE_DEST' ? destPrivate : undefined,
      };
    ...
    <div className="p-6 space-y-5 overflow-y-auto max-h-[70vh]">
      ...
      <section className="space-y-2">
        <div className="flex items-center gap-1.5 text-xs font-semibold text-zinc-900">
          Destination strategy
          <InfoTooltip title="Destination strategy" whatIsIt="Option 1 creates destination repos at the start of each mirror job (with an editable per-repo name). Option 2 mirrors into repositories you already created." />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <button
            type="button"
            onClick={() => setMode('CREATE_DEST')}
            className={`p-3 rounded-xl border text-left transition-colors ${mode === 'CREATE_DEST' ? 'border-zinc-900 bg-zinc-50 ring-1 ring-zinc-900' : 'border-zinc-200 bg-white hover:bg-zinc-50'}`}
          >
            <div className="text-xs font-semibold text-zinc-900">1 · Create destination repos</div>
            <p className="text-[11px] text-zinc-500 mt-1">Same name by default (editable). Created automatically when the mirror job starts.</p>
          </button>
          <button
            type="button"
            onClick={() => setMode('USE_EXISTING')}
            className={`p-3 rounded-xl border text-left transition-colors ${mode === 'USE_EXISTING' ? 'border-zinc-900 bg-zinc-50 ring-1 ring-zinc-900' : 'border-zinc-200 bg-white hover:bg-zinc-50'}`}
          >
            <div className="text-xs font-semibold text-zinc-900">2 · Use existing destinations</div>
            <p className="text-[11px] text-zinc-500 mt-1">Pick repositories that already exist. Non-empty ones are flagged for your decision.</p>
          </button>
        </div>
      </section>

      {/* Destination config (Option 1) */}
      {mode === 'CREATE_DEST' && (
        <section className="p-3 bg-zinc-50/70 border border-zinc-200 rounded-xl space-y-2">
          <div className="text-xs font-semibold text-zinc-900">Destination for created repos</div>
          <div className="grid grid-cols-2 gap-2">
            <select
              value={destCredentialId}
              onChange={(e) => {
                setDestCredentialId(e.target.value ? Number(e.target.value) : '');
                setDestOwner('');
                setDestInstallationId(undefined);
              }}
          ...
          {destCreateBlocked && (
            <div className="flex items-start gap-1.5 p-2 bg-amber-50 border border-amber-200 rounded-lg text-[10px] text-amber-800">
              ...
              <span>
                The selected App installation lacks <span className="font-semibold">Administration (write)</span> permission —
                repository creation would be rejected (403). Pick another installation or use a PAT with repo-create scope.
              </span>
            </div>
          )}
          {!destCreateBlocked && destOwnerInstallMissing && (
            <div className="flex items-start gap-1.5 p-2 bg-amber-50 border border-amber-200 rounded-lg text-[10px] text-amber-800">
              <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
              <span>
                The App is not installed for “{effectiveOwner}” — creation could land under a different account.
                Pick an installation from the list above.
              </span>
            </div>
          )}

          <label className="flex items-center gap-2 bg-white border border-zinc-200 rounded-xl px-3 py-2 text-xs w-fit">
            <input type="checkbox" checked={destPrivate} onChange={(e) => setDestPrivate(e.target.checked)} disabled={!publicReposEnabled} />
            Private
          </label>
        </section>
      )}
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <div className="text-xs font-semibold text-zinc-900">
            Repositories {rows.length > 0 && <span className="text-zinc-400">({rows.length})</span>}
          </div>
          <div className="flex items-center gap-2">
            ...
            {mode === 'USE_EXISTING' && rows.length > 0 && (
              <button
                type="button"
                onClick={() => setPickerOpen('DEST')}
                className="px-3 py-1.5 rounded-lg border border-zinc-300 text-zinc-700 text-xs font-medium hover:bg-zinc-50"
              >
                Pick destinations
              </button>
            )}
        ...
        {rows.length === 0 ? (
          <div className="py-8 text-center text-zinc-400 text-xs border border-dashed border-zinc-300 rounded-xl">
            Select one or more source repositories. Multi-select works across pages and searches.
          </div>
        ) : (
          <div className="border border-zinc-200 rounded-xl overflow-hidden">
            <table className="w-full text-xs">
              <thead className="bg-zinc-50 text-zinc-500">
                <tr>
                  ...
                  <th className="text-left px-3 py-2 font-medium">
                    {mode === 'CREATE_DEST' ? 'Destination name (editable)' : 'Existing destination'}
                  </th>
                  <th className="w-8" />
                </tr>
              </thead>
                        ...
                        {mode === 'CREATE_DEST' ? (
                          <div className="space-y-1">
                            <input
                              type="text"
                              value={row.destName}
                              onChange={(e) => setRows((prev) => prev.map((r, i) => (i === idx ? { ...r, destName: e.target.value } : r)))}
                          ...
                          <span className="text-zinc-400">— pick destinations above —</span>
                        )}
                      </td>
                      <td className="px-1">
                        <button
        ...
        {mode === 'USE_EXISTING' && nonEmptyUndecided > 0 && (
      ...
      <section className="p-3 bg-zinc-50/70 border border-zinc-200 rounded-xl space-y-2">
        ...
        <div className="grid grid-cols-4 gap-2">
          ...
          <label className="text-[10px] text-zinc-500 space-y-1">
            Direction
            <select value={syncDirection} onChange={(e) => setSyncDirection(e.target.value as SyncDirection)} className="w-full bg-white border border-zinc-200 rounded-lg px-2 py-1.5 text-xs">
              <option value="BIDIRECTIONAL">Bidirectional</option>
              <option value="UNIDIRECTIONAL_A_TO_B">A → B only</option>
              <option value="UNIDIRECTIONAL_B_TO_A">B → A only</option>
            </select>
          </label>
          <label className="text-[10px] text-zinc-500 space-y-1">
            Trunk conflicts
      ...
      <RepoPickerModal
        ...
        onSelectRepo={() => {}}
        title={pickerOpen === 'DEST' ? 'Select existing destination repos' : 'Select source repositories'}
        access={pickerOpen === 'DEST' ? 'PUSH' : 'PULL'}
        multiSelect
        initialSelection={pickerOpen === 'DEST'
          ? rows.map((r) => r.destRepo).filter(Boolean) as GitHubRepoOption[]
          : rows.map((r) => r.repo)}
        onConfirmMulti={pickerOpen === 'DEST' ? addDestRepos : addSources}
