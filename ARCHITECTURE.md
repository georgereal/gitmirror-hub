...
In bidirectional setups, simultaneous commits to the same branch could create split-brain divergences.

- **Fast-Forward Detection**: JGit's `RevWalk.isMergedInto()` verifies whether the destination remote head is an ancestor of the incoming commit. The same check runs on **every existing branch** (incremental webhook jobs and full-mirror), not only trunks. A new branch with no destination tip is still created. A remirror never force-overwrites a diverged head.
...
- **Releases & Binary Assets**: `ReleaseAndStatusSyncService` mirrors releases **destination-side** on full-mirror jobs and the dedicated Sync Releases action, never on single-branch Git jobs. Both sides are listed with **cursor-paged walks** (GraphQL `releases(first:, after:)` endCursor on GitHub/GHES; REST Link/`page` markers elsewhere) — the two listing chains run **concurrently** on the `releaseSyncExecutor` pool, then per-release work (CREATE missing, UPDATE drifted name/body/draft/prerelease, stream missing binary assets) fans out on the same bounded pool (`GIT_RELEASE_CONCURRENCY`, default 4). Assets stream **source → temp file → destination upload**, each hop binding its own side's credential. Every adapter call binds `ScmCredentialContext` (pair credential **+ App installation**), so multi-install Apps mint the install that owns the repo; a missing/unusable destination credential fails the stage loudly (`MetadataSyncException`) instead of silently returning zero. Providers without a Releases API (Bitbucket) degrade to an explicit `UNSUPPORTED` notice; GitLab releases create/update via API v4 (binaries are links, not uploads). Per-release mirror status (`MIRRORED` / `PENDING` / `UNSUPPORTED`) is computed against the real destination listing and rendered in the pair UI; pair snapshots persist true source/destination counts.
- **CI Checks & Commit Statuses**: `ReleaseAndStatusSyncService.syncCiChecks` backfills CI state onto mirrored tip commits — tips default to the most recently committed heads in the local bare mirror (trunk first, capped by `GIT_CI_CHECK_TIP_LIMIT`, default 8). GitHub/GHES destinations receive **native check runs** (`POST /repos/{o}/{r}/check-runs`, idempotent by name + conclusion); GitLab/Bitbucket mirror checks as commit/build statuses. Legacy commit statuses replicate newest-state-per-context only when the destination state differs. Real-time webhook status replication (`replicateCommitStatus`) binds the target credential + installation. Diff inspection lists destination check runs/statuses for idempotency and shows live trunk check runs in the CI Checks tab.
- **Metadata credential discipline**: All release/CI/status adapter calls — sync, diff inspection (`GitComparisonService`), and standalone actions — go through `ScmCredentialContext.open(credentialId, installationId)` per side. Hard auth failures (401/403, installation mismatch) surface as `FAILED` pipeline stages with ERROR audit rows; they are never swallowed.
...
- **Operator queue control**: Each execution lane (`git.sync.queue` full mirrors, `git.sync.incremental.queue` webhook syncs) has concurrency 1. Different pairs can run a full clone and a webhook incremental sync at the same time; the same pair still serializes on a shared `RepoDirLockService.lockFor(mappingId)` entry (the one bare-repo mutex also taken by `GitComparisonService` refresh-mode diffs — see §3.10) **plus a DB** `pair_leases` **row** so multi-pod replicas cannot double-write the same mapping. Operators cancel queued jobs (skipped on pickup and ACK'd, never DLQ'd), abort an in-flight run via JGit `ProgressMonitor.isCancelled()` without interrupting the AMQP listener thread, or purge the waiting AMQP buffer from `/queues`.
...
- **Bare-repo fetch verification & mutual exclusion** (`[future-work/fetch-ref-verification-and-repo-dir-mutex.md](future-work/fetch-ref-verification-and-repo-dir-mutex.md)`): JGit's `fetch().call()` returns normally even when individual per-ref writes fail (`LOCK_FAILURE`, `IO_FAILURE`, `REJECTED`); `BareRepoHousekeeping.logFailedTrackingRefUpdates` inspects every `FetchResult` at the sync-engine and diff-refresh fetch sites, WARNs per failed ref, and accumulates them on `SyncResult.failedFetchRefs` instead of letting a job report SUCCESS over a stale tip. Refresh-mode diffs hold the shared `RepoDirLockService` per-mapping lock, so a diff fetch can never race a sync job's fetch/push on the same bare repo directory.
...
- **Bulk mirror bootstrap (blank destination)**: For full-mirror jobs whose destination advertises **zero refs**, the engine pushes every remaining ref in a **single connection** (git `fetch --all` + `push --mirror` semantics, minus `refs/pull/`*, remote-tracking, and hub-managed namespaces, and with **no delete specs**). The path is decided at push time from the destination's actual ref state (re-derived from the INSPECT_DEST target fetch — never a stored flag), announced in the audit log + `push_dest` pipeline detail, and pre-announced in the pair UI via `PermissionCheckReport.emptyDestination`. There is **no fallback**: a rejected ref or transport failure fails the job like any other push error (disable with `git-utility.git.push-bulk-bootstrap=false` / `GIT_PUSH_BULK_BOOTSTRAP`). Destinations with existing refs keep the batched precision push above.
- **Pipeline & provider API usage**: `SyncPipelineState` is persisted on `sync_jobs.pipeline_json` and live on `JOB_PROGRESS`. Stage order: `fast_path → verify_dest → fetch_source → inspect_dest → conflict_check → push_dest → pr_metadata → releases → ci_checks → lfs`. Release and CI-check failures mark their stage `FAILED` (with ERROR audit rows) instead of a silent "Completed with notice". `ProviderRateMeter` counts job-scoped REST calls (ThreadLocal `jobId`) separately from Git smart HTTP fetch/push-batch ops. Snapshot fields on `SyncJob` feed the Logs details panel after the run. Standalone **Sync Releases** / **Sync CI Checks** actions create real `MANUAL` `SyncJob`s (async on `syncTaskExecutor`) so Queue Manager, the pipeline stepper, and the audit log show live progress; their audit rows dual-write through `SyncAuditLogRepository`.
...
GitHub.com and GHES no longer share one god-row. `scm_credentials` stores one usable identity per row (GitHub App with **one or more selected installations**, or a PAT). Auth mode is exclusive: App rows never fall back to a leftover PAT. Each App card stores `installationIds[]` (snapshot from List installs / Select all listed — new orgs are not auto-included). Each pair side stores `sourceCredentialId` / `targetCredentialId` and, when a repo is picked under an App, `sourceInstallationId` / `targetInstallationId` so sync mints the install that owns that repo. Repo picker shows App-level credentials; search unions `/installation/repositories` across selected installs. GHES host URL lives **on that card**; multiple appliances are multiple cards. Webhooks: `/api/v1/webhooks/github/credential/{id}` (and `/ghes/credential/{id}`) with per-credential HMAC; Worker optional `WEBHOOK_SECRETS_JSON` map. Wrong install / moved org fails with `AUTH_INSTALLATION_MISMATCH` until the user rebinds — no auto-switch to installations[0].
...
Domain code publishes operator jobs through `SyncEventBus` (`QueueProducerService`); Rabbit listeners live under `messaging.rabbit` and load only when provider=`rabbitmq`. `GIT_MESSAGING_PROVIDER=kafka` still fails fast.
...
Operator **Activate DR** is one control per provider pair, on Settings → Disaster recovery (`GET /api/v1/dr-lanes`, `POST /api/v1/dr-lanes/activate`, `POST …/fail-back`, `POST …/probe`). A lane is every repository that shares the same source host and destination host. Organizations and enterprises are the lock targets inside that lane. Repositories are the detail list.

The lock is written in this order: an enterprise ruleset when the demoted credential has an enterprise slug, otherwise an organization ruleset for each org in the lane, and a repository ruleset (`gitmirror-replica-readonly`) only when those scopes cannot be written (permissions, or GHES with no enterprise API). Hub unlocks the live side even when the old source is unreachable, flips incremental direction toward the demoted source, and parks git and metadata webhooks in `failover_parked_event`. A dark peer is that hold ledger, not the DLQ. Kafka deliveries are acked when parked so the partition does not stall.

Processing stays closed until both peer heartbeats are UP and the demoted lock is applied. Heartbeats default to 120s (`GIT_PEER_HEARTBEAT_SECONDS` / `GIT_PEER_HEARTBEAT_MS`) and are shared per git host and credential. While the lock is pending, the heartbeat keeps probing the dark host and retries the same enterprise, org, or repository lock. When the host answers, Hub applies that lock and then drains parked events in receive order. **Fail back** requires both hosts up, locks the side that was writable during DR, restores the previous direction, and drains.

The lane page shows each provider as up or down. The arrow aimed at a down provider is the broken direction. **Ruleset updates** lists enterprise, organization, and repository rows as applied, waiting, write-open, or not written. The lane rollup is green when every target at the chosen scope is applied, yellow when some are, red when a failover lock is still waiting, and unset when no ruleset has been written. Pair pages show status for that repository and link back to the lane. They do not activate DR on their own.

Settings → Metadata sync (`GET`/`PUT /api/v1/metadata-sync-settings`) is a global gate, default on, for pull requests, releases and assets, CI checks (commit status and completed check runs), and Git LFS. Off skips that full-mirror stage, drops the matching webhook, and disables the pair-page action. Git ref push stays on. LFS on an incremental push scans pointers between the before and after SHAs. `check_suite` is not ingested.

Settings → Replica rulesets (`GET`/`POST /api/v1/write-authority`) can place that lock on the repository, on the organization (`POST /orgs/{org}/rulesets`), or on a GitHub Enterprise Cloud enterprise (`POST /enterprises/{slug}/rulesets`). The setup steps and the ruleset JSON are in `INSTRUCTIONS.md`. The enterprise slug is stored on the GitHub.com App credential. GHES stops at the organization. A linked pair locks the side that is becoming read-only before it opens the other side. Org and enterprise rulesets default to the one repository; covering every repository requires the operator to confirm `ALL REPOS`. The App needs organization Administration write for an org ruleset. A 403 on the enterprise API leaves that scope disabled.
...
## 10. Database Persistence & Multi-Store Architecture

Exactly **one** persistence store is active per Hub process, selected by env and enforced at boot — no fallbacks, no dual wiring (mirrors the `messaging/` provider pattern):


| Provider         | Selector                         | Stack                                                                                           | Use                                                                                 |
| ---------------- | -------------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| **H2** (default) | `GIT_PERSISTENCE_PROVIDER=h2`    | Spring Data JPA + file H2 (`jdbc:h2:file:./data/gitutility;DB_CLOSE_DELAY=-1;AUTO_SERVER=TRUE`) | Dev and local multi-pod smoke (`AUTO_SERVER` lets multiple Hub JVMs share one file) |
| **MongoDB**      | `GIT_PERSISTENCE_PROVIDER=mongo` | Spring Data MongoDB (`MONGODB_URI`, default `mongodb://localhost:27017/gitutility`)             | Enterprise scale-out                                                                |




### Architecture (future-work/done/multi-store-persistence-h2-mongo.md)

- **Facade layer** (`repository/`): one plain interface per aggregate (15) — services, controllers, and provider adapters inject facades only.
- **Provider stores**: `repository/h2/` (`XJpaRepository` + `H2XStore`, `@OnH2`) and `repository/mongo/` (`XMongoRepository` + `MongoXStore`, `@OnMongo`). Exactly one store bean per facade is registered; `PersistenceEnvironmentPostProcessor` excludes the inactive stack's autoconfiguration symmetrically.
- **Shared domain model**: each entity carries both `@Entity` and `@Document` — one model, two stores. Repository scanning is scoped per provider (`H2RepositoriesConfig` / `MongoRepositoriesConfig`) because dual-annotated entities confuse Boot's multi-store auto-scan.
- **IDs**: ObjectId-as-String (24-char hex) end-to-end — REST paths, AMQP payloads, WebSocket events, UI. Generated by `Ids.newId()` during write preparation.
- **Secrets**: `@Encrypted` fields are AES-256-GCM encrypted at rest by **both** stores (`EncryptedStringConverter` for JPA, `MongoEntityLifecycleListener` for Mongo) — same `enc:v1:` ciphertext, cross-store decryptable.
- **Store-specific query ports**: the non-portable JPQL methods (dynamic job search + lane logic with `TRIM` semantics, lease renew/release, URL/regex mapping match, retention deletes) are reimplemented in the Mongo stores via `MongoTemplate`. Lease takeover on Mongo is atomic (`updateOne` on owner+expiry) — stricter than the JPA read-modify-write.
- **Descriptor**: `GET /api/v1/persistence` (mirrors `GET /api/v1/messaging`).



### Schema evolution & breaking changes

- **H2**: `DatabaseSchemaMigrator` (`@OnH2`) runs idempotent `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` checks at boot; `ddl-auto: update` creates the schema.
- **MongoDB**: `MongoSchemaInitializer` (`@OnMongo`) pings (fail-fast — an unreachable MongoDB aborts startup, no fallback) and creates unique (`repo_mappings.name`, `sync_jobs.queueMessageId`, `ref_origins.mappingId+refName`), compound, and TTL indexes (7-day unmapped-webhook retention).
- **Breaking-change note**: ids are ObjectId-strings — existing H2 file databases (BIGINT ids) are NOT compatible: delete `./data/` (or the configured data dir) before first boot. Queued AMQP messages from a pre-upgrade build reference numeric ids and will dead-letter — drain queues before deploying.
...
1. **Contract Isolation (**`ScmProviderAdapter`**)**: Standardized interface encapsulating credentials resolution (credential + installation aware), permission verification, workspace repo browsing, remote repository auto-creation, pull request lifecycle management, commit status replication, cursor-paged release listing (`listReleasesPage`), release mirror mutations (`findReleaseByTag` / `createRelease` / `updateRelease` / `uploadReleaseAsset` / `downloadReleaseAsset`), CI check run paging + creation (`listCiCheckRunsPage` / `createCheckRun` / `listCommitStatuses`), and capability flags (`supportsReleaseSync` / `supportsCheckRunSync`).
...
1. **GitHub Cloud & GHES (**`/webhook/github`**,** `/webhook/ghes`**)**:
  ...
  - Accepts `push`, `create`, `delete`, `pull_request`, `release`, `status`, and `check_run`. Anything else, including `check_suite`, returns `200 ignored` and never reaches Hub. On a GitHub App, `check_run` is not a Subscribe-to-events checkbox: **Checks: Read and write** subscribes the App to `check_run` and `check_suite` automatically ([`SCM_PROVIDER_SETUP.md`](SCM_PROVIDER_SETUP.md) §1.3). Redeploy `webhook-worker-kafka` when `GIT_WEBHOOK_BUS_PROVIDER=kafka`, or `webhook-worker` when the bus is RabbitMQ. A Hub restart does not update the Worker filter.
  - `push`, `create`, and `delete` stay on the git lane. `pull_request`, `release`, `status`, and `check_run` carry the raw body and are mirrored on the opposite side, unless that switch is off in Settings → Metadata sync. `push` also scans the pushed commit range for Git LFS pointers when LFS is on.
  - If the pair is in disaster recovery and either peer is down, or the demoted lock is still pending, the consumer parks the event in `failover_parked_event` and acks. It does not nack into the DLQ for connection refused, timeout, or HTTP 502/503/504.
